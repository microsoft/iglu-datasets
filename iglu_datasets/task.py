...
class Task:
    def __init__(self, instruction, target_grid, dialog=None, starting_grid=None, full_grid=None, invariant=True):
        ...
        Args:
            ...
            dialog : str
                Contatenation of all past utterances (excluding `instruction`), 
                represents the dialog context, by default is None
            ...
            invariant : bool, optional
                whether to calculate maximal intersection over rotations/translations. 
                by default True
        """
    ...
    def __repr__(self) -> str:
        instruction = self.instruction \
            if len(self.instruction) < 20 \
            else self.instruction[:20] + '...'
...
class Tasks:
    ...
    def to_dense(cls, blocks):
        """
        TODO: add note on the data format
        """
...
class Subtasks(Tasks):
    ...
    def create_task(self, turn_start: int, turn_goal: int):
        ...
        """
        dialog = ''
        for turn in self.dialog[:turn_goal]:
        ...
        task = Task(
            ...
            dialog=dialog, target_grid=self.to_dense(target_grid),
